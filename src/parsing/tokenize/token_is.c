...
/*   Updated: 2025/11/04 16:46:23 by modiepge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	token_is_redirect(t_token *token)
{
	if (!token)
		return (0);
	return (token->is_quoted == 0 && (token->type == TOK_DOUBLE_LESS
			|| token->type == TOK_DOUBLE_GREATER || token->type == TOK_GREATER
			|| token->type == TOK_LESS));
}
...
int	token_is_operator(t_token *token)
{
	if (!token)
		return (0);
	return (token->is_quoted == 0 && (token->type == TOK_PIPE
...
int	token_is_separator(t_token *token)
{
	if (!token)
		return (0);
	return (token->is_quoted == 0 && (token_is_operator(token)
			|| token_is_parenthesis(token)));
}
