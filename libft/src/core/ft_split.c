...
/*   Updated: 2025/07/11 16:23:22 by modiepge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**hlp_index(const char *s, char delimiter)
{
	...
	index = malloc(sizeof(char *) * (items + 1));
	if (index)
		index[items] = NULL;
	return (index);
}

static void	hlp_freeindex(char **index, size_t items)
{
	while (items != 0)
		free(index[--items]);
	free(index);
...
char	**ft_split(const char *s, char c)
{
	...
	while (s[cursor] && index)
	{
		...
		{
			index[item] = ft_substr(s, cursor, hlp_itemlen(&s[cursor], c));
			if (!index[item])
			{
				hlp_freeindex(index, item);
				return (NULL);
			}
			item++;
		}
